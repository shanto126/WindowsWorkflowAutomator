...
internal static class AppComposition
{
    public static IHost BuildHost()
    {
        ...
        var licenseServerUrl = builder.Configuration[
            "Application:LicenseServerUrl"];
        var licenseHttpClient = new HttpClient
        {
            BaseAddress = Uri.TryCreate(
                licenseServerUrl,
                UriKind.Absolute,
                out var licenseServerUri)
                ? licenseServerUri
                : new Uri("http://localhost:5078"),
            Timeout = TimeSpan.FromSeconds(3)
        };
        builder.Services.AddSingleton(licenseHttpClient);
        builder.Services.AddSingleton<AuthApiClient>();
        builder.Services.AddSingleton<AppSession>();
        builder.Services.AddSingleton<SubscriptionApiClient>();
